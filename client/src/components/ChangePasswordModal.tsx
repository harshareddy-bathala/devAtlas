...
type PasswordMethod = 'password' | 'email';
...
function ChangePasswordModal({ isOpen, onClose }: ChangePasswordModalProps) {
  const [method, setMethod] = useState<PasswordMethod>('password');
  ...
  const handleChangePassword = async (): Promise<void> => {
    ...
    try {
      ...
      if (authError.code === 'auth/wrong-password' || authError.message?.includes('wrong-password')) {
        ...
        setError(authError.message || 'Failed to change password. Please try again.');
      }
    } finally {
      setLoading(false);
    }
