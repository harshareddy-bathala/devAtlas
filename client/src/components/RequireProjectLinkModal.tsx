...
import { X, Loader2, Check, AlertCircle, FolderKanban, Plus } from 'lucide-react';
...
interface Skill {
  ...
  status: string;
  icon?: string;
  linkedProjects?: string[] | Record<string, string>;
}

interface RequireProjectLinkModalProps {
  isOpen: boolean;
  onClose: () => void;
  skill: Skill | null;
...
function RequireProjectLinkModal({ isOpen, onClose, skill, onLinked }: RequireProjectLinkModalProps) {
  ...
  const loadProjects = async (): Promise<void> => {
    ...
    try {
      ...
      const existingLinked = skill?.linkedProjects;
      const linkedArray = Array.isArray(existingLinked) 
        ? existingLinked 
        : (existingLinked ? Object.values(existingLinked) : []);
      setSelectedProjects(linkedArray.filter(id => completedIds.includes(id)));
    } catch (error) {
