...
function Onboarding(): JSX.Element {
  ...
  const [usernameStatus, setUsernameStatus] = useState<UsernameStatus>({ checking: false, available: null, reason: null });
  const [saving, setSaving] = useState<boolean>(false);
  const [touched, setTouched] = useState<TouchedFields>({ displayName: false, username: false });
  ...
  useEffect(() => {
    const handlePopState = async (e: PopStateEvent): Promise<void> => {
      e.preventDefault();
      // Sign out the user
      try {
        await signOut();
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="space-y-5">
                  {/* Display Name */}
                  <div>
                    <label htmlFor="displayName" className="flex items-center gap-2 text-sm font-medium text-white mb-2">
                    ...
                    <input
                      ...
                      aria-invalid={touched.displayName && !formData.displayName.trim()}
                      aria-describedby="displayName-error"
                      className={`w-full px-4 py-3 bg-transparent border rounded text-white placeholder-[#666] focus:outline-none focus:ring-2 transition-all ${
                      ...
                      maxLength={50}
                    />
                    <div id="displayName-error" role="alert" aria-live="polite">
                      {touched.displayName && !formData.displayName.trim() && (
                  ...
                  {/* Username */}
                  <div>
                    <label htmlFor="username" className="flex items-center gap-2 text-sm font-medium text-white mb-2">
